...
def get_cluster_center(ec2id, data_path, epoch):
    ...
    if epoch != 0:
        ...
        for ec in tqdm(list(ec2id.keys())):
            avg_pro_feas = []
            for pro_id in ec2id[ec]:
                pro_feature = protein_dict[pro_id]['pro_feat']#f[pro_id][:]
...
def embedding(sequence_name, data_path):
    ...
    return evo_feature.astype(np.float32)
...
class ProDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return sequence_name, sequence, label, EC1.astype(np.int), graph, evo_feature
...
class GCN(nn.Module):
    ...
    def forward(self, adj, evo_fea):
        _layers = []
        local_fea = self.act_fn(self.fcs[0](evo_fea))
...
class SCREEN(nn.Module):
    def __init__(self, nlayers, nfeat, nhidden, nclass, dropout):
        ...
        self.projection1 = nn.Linear(nhidden//2, nhidden//4)
        self.projection2 = nn.Linear(nhidden//4, nclass)
        self.act_fn =nn.ReLU()
        self.optimizer = torch.optim.Adam(self.parameters(), lr=LEARNING_RATE, betas=(0.9, 0.999))
        self.predict_ec = predict_ec(nhidden)

    def forward(self, adj, evo_fea):
        enz_feas = self.gcn( adj, evo_fea)
